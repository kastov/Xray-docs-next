...
По умолчанию Xray при изначальном запуске использует кастомный JSON-парсер (который вырезает из конфигурации комемнтарии и прочие нестандартные символы). Если вы уверены, что ваш конфигурационный файл строго соответствует JSON-стандарту (RFC8259), вы можете включить этот параметр, чтобы использовать стандартный JSON-парсер, который при больших конфигурациях будет работать быстрее.
